...
# screen -dmS cmt bash -c "bash script/combined_perturb.sh > ./log/combined_perturb_log_$ts.log 2>&1"
    ...
    for CURRENT_DATASET_NAME in "${DATASET_NAMES[@]}"; do
        ...
        for GROUP in "${DATASET_GROUPS[@]}"; do
            ...
            if [ -d "$PERTURB_MODEL_PATH" ]; then
                ...
                python -m train.sft_wrong --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
...
###
            ...
            python -m perturb.reason_perturb --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --cot_memory_perturb_same True
            CLIENT_EXIT_CODE=$?
            echo "Client script finished with exit code $CLIENT_EXIT_CODE."
