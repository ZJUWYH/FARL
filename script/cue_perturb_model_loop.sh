...
# bash /data/yuhui/8/memory-perturb/script/cue_perturb_model_loop.sh > "$LOGFILE" 2>&1
# ts=$(date '+%Y%m%d_%H%M%S')
# screen -dmS mt bash -c "bash /data/yuhui/8/memory-perturb/script/cue_perturb_model_loop.sh > ./log/cue_p_log_$ts.log 2>&1"
...
for model_pair in "${MODEL_PAIRS[@]}"; do
    ...
    for GROUP in "${DATASET_GROUPS[@]}"; do
        ...
        python cue.py --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
        CLIENT_EXIT_CODE=$?
        echo "Client script finished with exit code $CLIENT_EXIT_CODE."
