...
# bash /data/yuhui/8/memory-perturb/script/cot_quality2.sh > "$LOGFILE" 2>&1
# ts=$(date '+%Y%m%d_%H%M%S')
# screen -dmS mt bash -c "bash /data/yuhui/8/memory-perturb/script/cot_quality2.sh > ./log/cot_quality_log_$ts.log 2>&1"
...
for model_pair in "${MODEL_PAIRS[@]}"; do
    ...
    for GROUP in "${DATASET_GROUPS[@]}"; do
        ...
        python cot_quality_analysis.py --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --group_name "$GROUP"
        ...
        # CUDA_VISIBLE_DEVICES=0,1 python -m train.sft_wrong_7 --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
