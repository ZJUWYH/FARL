...
# bash /data/yuhui/8/memory-perturb/script/steering_loop.sh > "$LOGFILE" 2>&1
# ts=$(date '+%Y%m%d_%H%M%S')
# screen -dmS edit bash -c "bash /data/yuhui/8/memory-perturb/script/steering_loop.sh > ./log/steering_log_$ts.log 2>&1"
...
for model_pair in "${MODEL_PAIRS[@]}"; do
    ...
    for GROUP in "${DATASET_GROUPS[@]}"; do
        ...
        # CUDA_VISIBLE_DEVICES=0,1,2,3 python get_activations2.py --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
        # # CUDA_VISIBLE_DEVICES=0,1,2,3 accelerate launch --config_file config/get_act_4process.yaml get_activations_acc.py --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
        # echo "--- Stage 1 Complete ---"
        # echo


        echo "--- Stage 2: filter activations by auc ---"
        echo "filter activations..."
        python act_cls.py --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
        echo "Done filtering activations."
        echo
