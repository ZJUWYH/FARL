...
# bash /data/yuhui/8/memory-perturb/script/attention_map.sh > "$LOGFILE" 2>&1
# ts=$(date '+%Y%m%d_%H%M%S')
# screen -dmS edit bash -c "bash /data/yuhui/8/memory-perturb/script/attention_map.sh > ./log/attention_map_log_$ts.log 2>&1"
...
for model_pair in "${MODEL_PAIRS[@]}"; do
    ...
    for GROUP in "${DATASET_GROUPS[@]}"; do
        ...
        DOUBLE_PERTURB_PATH="./res3/${DATASET_NAME}_${GROUP}_${DATASET_SPLIT}_cue_cot_perturb_perturb_{CURRENT_SHORT_MODEL_NAME}_perturbed_answers.json"
...
######################################################
        echo "--- Stage 1: get activations ---"
        python get_atten_map5.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
        echo "--- Stage 1 Complete ---"
        # echo
        echo "--- Stage 1: cls attention map ---"
        python cls_attention_map.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map"
        echo "--- Stage 1 Complete ---"
        echo
        #######################################################

        # echo "--- Stage 2: get activations ---"
        # python get_atten_map3_cot.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_cot"
        # echo "--- Stage 2 Complete ---"
        # echo

        # echo "--- Stage 2: cls attention map ---"
        # python cls_attention_map.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_cot"
        # echo "--- Stage 2 Complete ---"
        # echo

        # echo "--- Stage 3: get activations ---"
        # python get_atten_map3_memory.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_memory"
        # echo "--- Stage 3 Complete ---"
        # echo

        # echo "--- Stage 3: cls attention map ---"
        # python cls_attention_map.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_memory"
        # echo "--- Stage 3 Complete ---"
        # echo

        # echo "--- Stage 4: get activations ---"
        # python get_atten_map3_same.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_same"
        # echo "--- Stage 4 Complete ---"
        # echo

        # echo "--- Stage 4: cls attention map ---"
        # python cls_attention_map.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_same"
        # echo "--- Stage 4 Complete ---"
        # echo
#######################################################

        # echo "--- Stage 3: edit weight ---"
        # echo "edit weight..."
        # python edit_weight.py --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
        # echo "Done editing weight."
        # echo

        # EDITED_MODEL_PATH="./ckpt/edited_models/${GROUP}_${CURRENT_SHORT_MODEL_NAME}"

        # # --- STAGE 4: Inference with Edited Model ---
        # echo "--- Stage 4: Running Inference on Edited Model ---"
        # start_vllm_server "$EDITED_MODEL_PATH" "$CURRENT_SHORT_MODEL_NAME"
        # wait_for_server || { echo "Server failed to start, skipping to next iteration."; shutdown_vllm_server; continue; }

        # echo "Running the edited model inference script..."
        # python edit_infer_api.py --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
        # CLIENT_EXIT_CODE=$?
