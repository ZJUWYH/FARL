...
# bash /data/yuhui/8/memory-perturb/script/attention_map_copy.sh > "$LOGFILE" 2>&1
# ts=$(date '+%Y%m%d_%H%M%S')
# screen -dmS edit bash -c "bash /data/yuhui/8/memory-perturb/script/attention_map_copy.sh > ./log/attention_map_log_$ts.log 2>&1"
...
#         python get_atten_map3.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
#         echo "--- Stage 1 Complete ---"
#         echo
#         #######################################################

        echo "--- Stage 1: get activations ---"
        python get_atten_map3_cot.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$PERTURB_MODEL_PATH" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_combined_same"
        echo "--- Stage 1 Complete ---"
        echo

        echo "--- Stage 2: cls attention map ---"
        python cls_attention_map.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --save_dir "./atten_map_combined_same"
        echo "--- Stage 2 Complete ---"
        echo
