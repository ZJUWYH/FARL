...
# bash /data/yuhui/8/memory-perturb/script/unlearn_loop.sh > "$LOGFILE" 2>&1
# ts=$(date '+%Y%m%d_%H%M%S')
# screen -dmS mt bash -c "bash /data/yuhui/8/memory-perturb/script/unlearn_loop.sh > ./log/log_$ts.log 2>&1"
...
for model_pair in "${MODEL_PAIRS[@]}"; do
    ...
    for GROUP in "${DATASET_GROUPS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=0,1 python -m train.unlearn_npo --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME"
