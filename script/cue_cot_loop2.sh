...
# bash /data/yuhui/8/memory-perturb/script/cue_cot_loop2.sh > "$LOGFILE" 2>&1
# ts=$(date '+%Y%m%d_%H%M%S')
# screen -dmS ct bash -c "bash /data/yuhui/8/memory-perturb/script/cue_cot_loop2.sh > ./log/cue_cot_log_$ts.log 2>&1"
# this is the version use only one field of dataset
# version2 add different dataset, and add skip for first to stage
...
for model_pair in "${MODEL_PAIRS[@]}"; do
    ...
    for CURRENT_DATASET_NAME in "${DATASET_NAMES[@]}"; do
        ...
        for GROUP in "${DATASET_GROUPS[@]}"; do
            ...
            python cue_cot3.py --dataset_name "$DATASET_NAME" --group_name "$GROUP" --model_name "$CURRENT_MODEL_NAME" --short_model_name "$CURRENT_SHORT_MODEL_NAME" --cot_memory_perturb_same False
            CLIENT_EXIT_CODE=$?
            echo "Client script finished with exit code $CLIENT_EXIT_CODE."
