...
def main():
    ...
    if accelerator.is_main_process:
        wandb.init(project="MemoryPerturbGroup", name=f"{DATASET_NAME}_{GROUP_NAME}_{DATASET_SPLIT}_{SHORT_MODEL_NAME}", group="sft_correct")
        wandb.config.update(args)
    # print("Starting MMLU fine-tuning for Qwen3-8B on H100...")

    

    OUTPUT_DIR = f"./ckpt/{DATASET_NAME}_{GROUP_NAME}_{DATASET_SPLIT}_{SHORT_MODEL_NAME}_sft" # Output directory for model checkpoints
    # Ensure output directory exists
    ...
    training_args = TrainingArguments(
        ...
        per_device_train_batch_size=4,         # Larger batch size for H100
