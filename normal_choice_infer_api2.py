...
# CUDA_VISIBLE_DEVICES=0,1 python normal_choice_infer_api.py
...
def load_and_concat_dataset(group_name: str):
    ...
    datasets = [load_dataset(DATASET_NAME, field, split=DATASET_SPLIT) for field in dataset_fields]
...
if __name__ == "__main__":
    # 加载数据集
    parser = argparse.ArgumentParser(description="Run inference with a specified MMLU dataset field group.")
    parser.add_argument(
        "--group_name",
        type=str,
        required=True,
    ...
    dataset = load_and_concat_dataset(GROUP_NAME)
