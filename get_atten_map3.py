...
def get_llama_activations_pyvene(collected_model, collectors, prompt):
    ...
    # atten_tuple = output.attentions
    # atten = torch.stack(list(atten_tuple), dim=0)
    # atten = atten[:,0] # the first batch (batch size = 1)
    # atten = atten[:,:,-1,:] # the last token
...
def get_atten_map(model, prompt):
    ...
    atten = atten[:,:,-1,:] # the last token
    atten = atten.float().cpu().numpy()
    print(f"atten shape: {atten.shape}") # (num_layers, num_heads, seq_len)
    del output
...
def get_atten_map_prefill(model, prompt):
    ...
    atten = out.attentions  # list of len num_layers, each (B, H, 1, KV)
    atten = torch.stack(list(atten), dim=0)         # (L, B, H, 1, KV)
    atten = atten[:, 0, :, -1, :].float().cpu().numpy()  # (L, H, KV)
...
def extract_activations(model_name, short_model_name, group_name, dataset, save_dir='./features'):
    ...
    def get_activations_example(example):
        ...
        # atten_map = get_atten_map(model, tokenized_prompt)

        if example["success_cot_perturb_last"] == True:
            label = 1
        elif example["success_memory_perturb"] == True:
            label = 2
        else:
            label = 0

        return {
            "layer_wise_activations": layer_wise_activations,
            "head_wise_activations": head_wise_activations,
            "label": label,
        }

    def get_atten_map_example(example):
        question = example["question"]
        choices = example["choices"]
        answer_index = example["answer"]
        perturb_answer_index = example["perturb_answer"]
        ...
        formatted_prompt = example["input_perturbed_cots"] + example["perturb_model_answer"]
        tokenized_prompt = tokenizer(formatted_prompt, return_tensors="pt").input_ids[:, :-1]
        atten_map = get_atten_map_prefill(model, tokenized_prompt)
        ...
        return {
            "layer_head_atten_map": atten_map,
        }

    # def get_activations_batch(batch):
    #     out_layer_head_atten_map = []
    #     out_layer_wise_activations = []
    #     out_head_wise_activations = []
    #     out_labels = []
    ...
    #         perturb_model_answer = batch["perturb_model_answer"][i]
    ...
    #         prompt = f"""What is the correct answer to this question? Question:\n {question}\nChoices:\n{choice_str}\nOutput format: The correct answer is (A/B/C/D)."""
    #         formatted_prompt = batch["input_perturbed_cots"][i]
    #         tokenized_prompt = tokenizer(formatted_prompt, return_tensors="pt").input_ids[:, :-1]
            
    #         layer_wise_activations, head_wise_activations, _, atten_map = get_llama_activations_pyvene(
    #             collected_model, collectors, tokenized_prompt
    #         )
            
    #         if batch["success_cot_perturb_last"][i] == True:
    #             label = 1
    #         elif batch["success_memory_perturb"][i] == True:
    #             label = 2
    #         else:
    #             label = 0
            
    #         out_layer_head_atten_map.append(atten_map)
    #         out_layer_wise_activations.append(layer_wise_activations)
    #         out_head_wise_activations.append(head_wise_activations)
    #         out_labels.append(label)
        
    #     return {
    #         "layer_head_atten_map": out_layer_head_atten_map,
    #         "layer_wise_activations": out_layer_wise_activations,
    #         "head_wise_activations": out_head_wise_activations,
    #         "label": out_labels,
    #     }
    ...
    # dataset = dataset.map(get_atten_map_example)
...
def main(): 
    ...
    INPUT_PATH = f"./res3/{DATASET_NAME}_{GROUP_NAME}_{DATASET_SPLIT}_cue_cot_perturb_perturb_{SHORT_MODEL_NAME}_perturbed_answers.json"
    # if input_path is not exist, then use the input_path_same
    if not os.path.exists(INPUT_PATH):
        INPUT_PATH = f"./res3/{DATASET_NAME}_{GROUP_NAME}_{DATASET_SPLIT}_cue_cot_perturb_perturb_{SHORT_MODEL_NAME}_isSame_False_perturbed_answers.json"
    ...
    # layer_head_atten_map = dataset["layer_head_atten_map"]
    # np.save(f'{args.save_dir}/{SHORT_MODEL_NAME}_{GROUP_NAME}_layer_head_atten_map.npy', layer_head_atten_map)
    layer_wise_activations = np.array(dataset["layer_wise_activations"])
    np.save(f'{args.save_dir}/{SHORT_MODEL_NAME}_{GROUP_NAME}_layer_wise_activations.npy', layer_wise_activations)
    head_wise_activations = np.array(dataset["head_wise_activations"])
    np.save(f'{args.save_dir}/{SHORT_MODEL_NAME}_{GROUP_NAME}_head_wise_activations.npy', head_wise_activations)
    # np.savez_compressed(f'{args.save_dir}/{SHORT_MODEL_NAME}_{GROUP_NAME}_layer_head_atten_map.npz', **{f'item_{i}': item for i, item in enumerate(layer_head_atten_map)})

    
    print(f"Activations and attention map saved to {args.save_dir}/")
    # print(f"Shapes - Layer-head-atten-map: {layer_head_atten_map[0].shape}")
    print(f"         Layer-wise: {layer_wise_activations.shape}")
