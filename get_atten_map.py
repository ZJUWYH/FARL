...
def get_llama_activations_pyvene(collected_model, collectors, prompt):
    with torch.no_grad():
        ...
        output = collected_model({"input_ids": prompt, "output_hidden_states": True, "output_attentions": True})[1]
    ...
    return hidden_states[:,-1,:].squeeze(), head_wise_hidden_states, mlp_wise_hidden_states, atten
...
def main(): 
    ...
    layer_head_atten_map = np.array(dataset["layer_head_atten_map"])
    ...
    print(f"Shapes - Layer-head-atten-map: {layer_head_atten_map.shape}")
